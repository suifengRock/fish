...
func main() {
	...
	provinces := js.Get("provinces")
	provincesArray, err := provinces.Array()
	for index, _ := range provincesArray {
		obj := provinces.GetIndex(index)

		name, _ := obj.Get("name").String()
		id, _ := obj.Get("id").Int()
		fmt.Println(name, ":", id)

		cityObj := obj.Get("citys")
		cityArray, _ := cityObj.Array()
		for tmp, _ := range cityArray {
			siteList, _ := cityObj.GetIndex(tmp).Map()
			for num, name := range siteList {
				if len(num) < 2 {
					num = "0" + num
				}
